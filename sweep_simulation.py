...
def run_simulation(
    ...
    analytical_volume = (
        2 * np.pi * sigma**2 * max_depth * (1 - np.exp(-(radius**2) / (2 * sigma**2)))
    )
    ...
    z_noisy = z_deformed + np.random.normal(
        loc=0, scale=noise_std, size=z_deformed.shape
    )
    ...
    R_batch = load_rotations(
        ...
        rot_root_path="exhaustive-grid-search/data/rotations",
    )
    ...
    with torch.no_grad():
        for ind_dataloader, (voxelized_pts_padded, mins, orig_input_shape) in enumerate(
            my_dataloader
        ):
    ...
    reg_o3d = o3d.pipelines.registration.registration_generalized_icp(
        ...
        pci_o3d_pcd,
        adaptive_thr,
        estim_T_baseline.cpu().numpy(),
        criteria=criteria,
    )
    estim_T_baseline_refined = reg_o3d.transformation

    gt = np.linalg.inv(T_gt)
    ...
    pc_target_gdf = gpd.GeoDataFrame(
        geometry=gpd.points_from_xy(x, y, z), crs="EPSG:4548"
    )
    ...
    pc_source_gdf = gpd.GeoDataFrame(
        geometry=gpd.points_from_xy(x_refined, y_refined, z_refined), crs="EPSG:4548"
    )
    ...
    indices, distances = epoch_stable_source.kdtree.nearest_neighbors(
        epoch_stable_target.cloud, 1
    )
    ...
    if selected_hulls:
        inside_mask_refined = is_inside_selected_hulls_vectorized(
            selected_hulls, points_refined_2d
        )
        inside_mask_raw = is_inside_selected_hulls_vectorized(
            selected_hulls, points_raw_2d
        )
    else:
    ...
    dem1 = griddata(
        (filtered_raw_x, filtered_raw_y), z1_adj, (grid_x, grid_y), method="linear"
    )
    dem2 = griddata(
        (filtered_refined_x, filtered_refined_y),
        z2_adj,
        (grid_x, grid_y),
        method="linear",
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Sweep simulation parameters and output CSV results."
    )
    parser.add_argument(
        "--noise_list",
        type=str,
        required=True,
        help="Comma separated list of noise_std values in mm, e.g., '100,150,200'",
    )
    parser.add_argument(
        "--quantile_start",
        type=float,
        required=True,
        help="Starting quantile threshold in percent",
    )
    parser.add_argument(
        "--quantile_step",
        type=float,
        required=True,
        help="Step for quantile threshold in percent",
    )
    parser.add_argument(
        "--quantile_count", type=int, required=True, help="Number of quantile steps"
    )
    parser.add_argument(
        "--output_csv", type=str, default="results.csv", help="Output CSV file"
    )
    ...
    parser.add_argument(
        "--input_file",
        type=str,
        default="/home/sam/Documents/datasets/WHU/3-Mountain/combined.las",
        help="Path to the input LAS file",
    )
    parser.add_argument(
        "--shp_file",
        type=str,
        default="/home/sam/Documents/datasets/WHU/3-Mountain/combined_stable.shp",
        help="Path to the shapefile",
    )
    parser.add_argument(
        "--radius", type=float, default=25, help="Radius of the mining region"
    )
    parser.add_argument(
        "--max_depth", type=float, default=15, help="Maximum depth at the center"
    )
    parser.add_argument(
        "--sigma",
        type=float,
        default=None,
        help="Spread of the deformation (if not provided, computed as radius/2)",
    )
    parser.add_argument(
        "--device", type=str, default="cuda:0", help="Torch device (e.g. 'cuda:0')"
    )
    parser.add_argument(
        "--padding", type=str, default="same", help="Padding option for convolution"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--rotation_choice",
        type=str,
        default="AA_ICO162_S10_z_axis",
        help="Rotation choice",
    )
    parser.add_argument(
        "--max_iter", type=int, default=2048, help="Max iterations for ICP"
    )
    ...
    results_list = []
    for noise_val_mm in noise_list:
        ...
        for j in range(args.quantile_count):
            ...
            print(
                f"Running simulation for noise_std = {noise_val_mm} mm, quantile_thr = {quantile_val_percent}%"
            )
